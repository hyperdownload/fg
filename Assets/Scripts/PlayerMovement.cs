...
namespace EthanTheHero
{
	public class PlayerMovement : MonoBehaviour
	{
		...
		private Checkpoint2D lastCheckpoint;
		#endregion
		// life
		[Header("LifeSystem")]
		...
		void Update()
		{
			...
			{
				// Ejecutar lógica de muerte del personaje
				Debug.Log("Cancerbero cuando:");
				Respawn();
			}
		}
        ...
        private void run(float lerpAmount)
		{
				...
				if (!CheckForObjectInCollider(targetObject))//Implementing run
		...
		public void SetCheckpoint(Checkpoint2D checkpoint)
		{
		...
		private bool CheckForObjectInCollider(GameObject target)
		{
			if (target != null)
			{
				...
				if (collider != null)
				{
					...
					filter.SetLayerMask(LayerMask.GetMask("ColliderNo"));

					Collider2D[] collidersInside = new Collider2D[1];

					int count = collider.OverlapCollider(filter, collidersInside);
